...
int recover(CString filename) {
    ...
    for (counter = 0; isfount == 0 && counter < 500; counter++) {
        ...
        for (facount = 0; facount <= 480; facount = facount + 32) {
            if (compare(filename,dirbuff,facount)==1) {
    ...
    dec = CreateFile(_T("I:\\rec.jpg"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, 0, NULL);
